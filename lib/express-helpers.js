...
module.exports = function(app) {
	var helpers = {};
	var date_tag = helpers.date_tag = function(name, value , html_options) {
	...
	var time_tag = helpers.time_tag = function(name, value , html_options) {
	  if (! (value instanceof Date))
		value = new Date();
	...
	helpers.date_time_tag = function(name, date, time, html_options){
		return date_tag(name, date, html_options) + time_tag(name, time, html_options)
	}
	...
	helpers.submit_tag = function(idfor) {
		...
		html_options.value = idfor || 'Submit';
		return single_tag_for('input', html_options);
	};
	...
	//if the app is given then  export all the helpers
	if(app){
		var obj = {};
